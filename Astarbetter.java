...
//Java Program to implement A* Search Algorithm
   ...
   void tracePath(Cell[][] cellDetails, int cols, int rows, Pair dest){   //A* Search algorithm path
       ...
       while (!path.empty()) {
           Pair p = path.peek();
           path.pop();
           PixelGrid.colors[p.first][p.second] = Color.BLUE;
       }
   }

// A main method, A* Search algorithm to find the shortest path

   void aStarSearch(int[][] grid, int rows, int cols, Pair src, Pair dest){
...
//heuristic function                               cellDetails[neighbour.first][neighbour.second].h = hNew;
   ...
   public void main(String[] args) {
        ...
        Astarbetter N = new Astarbetter();
        N.aStarSearch(grid, Size, Size, src, dest);
